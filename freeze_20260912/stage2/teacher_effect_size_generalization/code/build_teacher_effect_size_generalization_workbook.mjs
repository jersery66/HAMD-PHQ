...
const OUTPUT_NAME = "老师要求_A方案选择与重复五折外推结果.xlsx";
const FONT = "Arial";
const NAVY = "#17365D";
const TEAL = "#0F6B78";
const BLUE = "#2F75B5";
...
async function main() {
  ...
  if (payload.selected_strategy !== "A") throw new Error("Workbook input does not select Strategy A");
  if (manifest.verification_status !== "PASS" || independent.status !== "PASS") throw new Error("Verification gate is not PASS");

  const workbook = Workbook.create();
  const selection = [...payload.selection_summary].sort((a, b) => a.cross_scale_effect_rank - b.cross_scale_effect_rank);
  ...
  styleTitle(sheet, "老师要求的方案选择与重复五折外推结果", "先用 A/B/C 的合并标准化总分 MAE 效应确定一种方案，再只对该方案做每个 AI 独立的 10 次重复 × 5 折外推检验。负的 ΔMAE 表示优于 C03。", 3);
  const kpis = [
    ["最终方案", "A｜按会议指定的总分 MAE 合并效应量标准排名第 1"],
    ["方案选择效应", `A ${selection[0].cross_scale_subject_weighted_gz.toFixed(3)}；B ${selection[1].cross_scale_subject_weighted_gz.toFixed(3)}；C ${selection[2].cross_scale_subject_weighted_gz.toFixed(3)}`],
  ...
  const forestBytes = await fs.readFile(path.join(outputDir, "figures", "F2_A_generalization_forest.png"));
  sheet.images.add({
  ...
  sheet.getRange("A55").values = [["图中负值表示 A 的总分 MAE 更低；显著性和 95%CI 以顶部结果表为准。"]];
  sheet.getRange("A55").format = { fill: LIGHT_BLUE, font: { size: 9, name: FONT }, wrapText: true };
  sheet.freezePanes.freezeRows(3);

  sheet = workbook.worksheets.add("01_方案选择");
  styleTitle(sheet, "A/B/C 方案选择：为什么最终确定 A", "同一受试者的三个 AI 先等权合成，避免把相关 AI 当成独立研究；PHQ 与 HAMD 的 Hedges g_z 再按有效受试者数合并。越负表示总分 MAE 改善越大。", 18);
  const selectionColumns = [
    { key: "cross_scale_effect_rank", label: "排名", format: "0" },
    { key: "strategy", label: "方案" },
    { key: "cross_scale_subject_weighted_gz", label: "两量表合并g_z", format: "0.000" },
    { key: "PHQ_mean_delta_MAE", label: "PHQ平均ΔMAE", format: "0.000" },
    { key: "HAMD_mean_delta_MAE", label: "HAMD平均ΔMAE", format: "0.000" },
    { key: "cells_delta_MAE_below_0", label: "改善单元/6", format: "0" },
  ...
  sheet.getRange("C5:C7").conditionalFormats.add("colorScale", { colors: ["#63BE7B", "#FFEB84", "#F8696B"], thresholds: ["min", "50%", "max"] });
  ...
  addMethodRows(sheet, 15, [
    ...
    { topic: "为什么不是 B", why: "B 的条目真实性证据更均衡，且出现唯一 Level2/test47 支持；但合并总分效应为 -0.346，小于 A 的 -0.387。按本次会议规则，B 不进入后续主检验。" },
    { topic: "A 的风险", why: "A 在既有探索中常伴随条目 NAE、条目绝对误差和误差抵消增加。因此 A 即便总分显著改善，也只能支持总分优化，不能写成逐条评分更准。" },
    { topic: "为什么不把六个 AI 单元直接做元分析", why: "同一受试者在多个 AI 下重复出现，六个效应并不独立。先在受试者内对 AI 等权合成，再跨两个量表合并，能避免伪精确。" },
  ], "SelectionRationale");
  sheet.freezePanes.freezeRows(4);

  sheet = workbook.worksheets.add("02_外推主结果");
  styleTitle(sheet, "A 方案：每个 AI 独立的重复五折外推结果", "PHQ 全 189 人；HAMD 全 99 人、主分析排除 H14，使用 HAMD16-core。正式推断单位为受试者；50 个重叠折仅描述稳定性。", 25);
  const primaryColumns = [
    ...
    { key: "selected_total_MAE", label: "A总分MAE", format: "0.000" },
    ...
    { key: "selected_weighted_kappa", label: "A weighted kappa", format: "0.000" },
    { key: "C03_weighted_kappa", label: "C03 weighted kappa", format: "0.000" },
    { key: "C03_retention_rate", label: "A中C03保留率", format: "0.0%" },
  ...
  styleTitle(sheet, "10 次重复 × 5 折的 held-out 效应分布", "每行是一折的描述性结果，共 6 个量表×AI 单元 × 50 折。由于重复折共享受试者，这 50 个值不作为独立样本做正式 t 检验。", 11);
  ...
  const foldColumns = [
    ...
    { key: "selected_MAE", label: "A MAE", format: "0.000" },
  ...
  styleTitle(sheet, "全队列 A 条件映射与外层路由稳定性", "全队列映射用于未来新增受试者的固定候选规则，不参与当前 OOF 效应估计。未通过外推检验的模型明确标记为保留 C03。HAMD H14 不进入主分析。", 23);
  ...
  const mapColumns = [
    ...
    { key: "full_cohort_selected_condition", label: "全队列A条件", width: 105 },
  ...
  addMethodRows(sheet, 3, [
    ...
    { topic: "为什么用配对检验", why: "A 与 C03 在同一 held-out 受试者上产生误差，配对比较能直接估计每个人的误差变化。10 次 OOF 先在受试者内平均，避免把重复结果当成独立样本。" },
    ...
    { topic: "能写到什么程度", why: "可写 PHQ/DeepSeek 与 PHQ/Qwen 在当前队列内具有总分 MAE 的跨受试者内部泛化证据；HAMD 和 PHQ/GLM 不支持。所有 A 结果的条目误差或抵消均未同步改善。" },
  ...
  await fs.writeFile(path.join(outputDir, "workbook_build_audit.json"), `${JSON.stringify({ status: "PASS", output: OUTPUT_NAME, sheets: sheetNames, chartCount: 0, imageCount: 1, sourceVerification: manifest.verification_status, independentVerification: independent.status, visualVerification: "pending external per-sheet render because Artifact Tool render timed out before first preview" }, null, 2)}\n`, "utf8");
